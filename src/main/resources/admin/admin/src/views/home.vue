<template>
	<div class="home-content">
		...
		<div class="statis-box">
			...
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('dingdanxinxi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-chujia3"></span>
				</div>
				<div class="right">
					<div class="num">{{dingdanxinxiCount}}</div>
					<div class="name">订单信息总数</div>
				</div>
			</div>
			<div id="statis3" class="statis3 animate__animated" v-if="isAuth('yunshujilu','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-qita3"></span>
				</div>
				<div class="right">
					<div class="num">{{yunshujiluCount}}</div>
					<div class="name">运输记录总数</div>
				</div>
			</div>
			<div id="statis4" class="statis4 animate__animated" v-if="isAuth('shangpinruku','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-liulan12"></span>
				</div>
				<div class="right">
					<div class="num">{{shangpinrukuCount}}</div>
					<div class="name">商品入库总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<div class="type4">
			...
			<div id="dingdanxinxiChart1" class="echarts2 animate__animated" v-if="isAuth('dingdanxinxi','首页统计')"></div>
			<div id="yunshujiluChart1" class="echarts3 animate__animated" v-if="isAuth('yunshujilu','首页统计')"></div>
			<div id="shangpinrukuChart1" class="echarts4 animate__animated" v-if="isAuth('shangpinruku','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		if(this.isAuth('dingdanxinxi','首页统计')){
			this.dingdanxinxiChat1();
		}
		this.getyunshujiluCount();
		if(this.isAuth('yunshujilu','首页统计')){
			this.yunshujiluChat1();
		}
		this.getshangpinrukuCount();
		if(this.isAuth('shangpinruku','首页统计')){
			this.shangpinrukuChat1();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'dingdanxinxiChart1',css:'animate__fadeInUp'},
				{id:'yunshujiluChart1',css:'animate__fadeInUp'},
				{id:'shangpinrukuChart1',css:'animate__fadeInUp'},
			]
...
// 1234 竖
		dingdanxinxiChat1() {
			this.$nextTick(()=>{
				...
				this.$http({
					url: `dingdanxinxi/value/shangpinmingcheng/goumaifeiyong`,
					method: "get",
				}).then(({ data }) => {
					if (data && data.code === 0) {
						let res = data.data;
		...
		getyunshujiluCount() {
			this.$http({
				url: `yunshujilu/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.yunshujiluCount = data.data
				}
			})
		},
// 1234 饼
		yunshujiluChat1() {
			this.$nextTick(()=>{
				...
				var yunshujiluChart1 = echarts.init(document.getElementById("yunshujiluChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						yunshujiluChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							yunshujiluChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		getshangpinrukuCount() {
			this.$http({
				url: `shangpinruku/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.shangpinrukuCount = data.data
				}
			})
		},
// 1234 漏
		shangpinrukuChat1() {
			this.$nextTick(()=>{
				...
				var shangpinrukuChart1 = echarts.init(document.getElementById("shangpinrukuChart1"),'macarons');
				this.$http({
					url: `shangpinruku/value/shangpinmingcheng/kucunshuliang`,
					method: "get",
				}).then(({ data }) => {
					if (data && data.code === 0) {
						let res = data.data;
						...
						// 使用刚指定的配置项和数据显示图表。
						shangpinrukuChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							shangpinrukuChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
...
<style lang="scss" scoped>
	.home-content {
		padding: 10px 30px;
		background: url(http://codegen.caihongy.cn/20250403/64856f2928c146988e5569449b39f222.png) no-repeat center top / cover;
		display: flex;
		width: 100%;
		min-height: 100vh;
