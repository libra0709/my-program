	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info' && !ro.yunshutupian" label="运输图片" prop="yunshutupian" >
					<file-upload
						tip="点击上传运输图片"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.yunshutupian?ruleForm.yunshutupian:''"
						@change="yunshutupianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item class="upload" v-else-if="ruleForm.yunshutupian" label="运输图片" prop="yunshutupian" >
					<img v-if="ruleForm.yunshutupian.substring(0,4)=='http'&&ruleForm.yunshutupian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.yunshutupian" width="100" height="100">
					<img v-else-if="ruleForm.yunshutupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.yunshutupian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.yunshutupian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				...
				<el-form-item class="date" v-if="type!='info'" label="监控时间" prop="jiankongshijian" >
					<el-date-picker
				...
				<el-form-item class="input" v-else-if="ruleForm.jiankongshijian" label="监控时间" prop="jiankongshijian" >
					<el-input v-model="ruleForm.jiankongshijian" placeholder="监控时间" readonly></el-input>
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'"  label="监控状态" prop="jiankongzhuangtai" >
					<el-select :disabled="ro.jiankongzhuangtai" v-model="ruleForm.jiankongzhuangtai" placeholder="请选择监控状态" >
						<el-option
							v-for="(item,index) in jiankongzhuangtaiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="监控状态" prop="jiankongzhuangtai" >
					<el-input v-model="ruleForm.jiankongzhuangtai"
						placeholder="监控状态" readonly></el-input>
				</el-form-item>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="监控详情" prop="jiankongxiangqing" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.jiankongxiangqing" label="监控详情" prop="jiankongxiangqing" >
				<span class="text">{{ruleForm.jiankongxiangqing}}</span>
			</el-form-item>
...
<script>
	...
	export default {
		...
		created() {
			this.ruleForm.jiankongshijian = this.getCurDate()
		},
		methods: {
			// 下载
			download(file){
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='yunshutupian'){
							this.ruleForm.yunshutupian = obj[o];
							this.ro.yunshutupian = true;
							continue;
						}
						if(o=='kucunshuliang'){
						...
						if(o=='jiankongzhuangtai'){
							this.ruleForm.jiankongzhuangtai = obj[o];
							this.ro.jiankongzhuangtai = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
			...
			// 提交
			async onSubmit() {
					if(this.ruleForm.yunshutupian!=null) {
						this.ruleForm.yunshutupian = this.ruleForm.yunshutupian.replace(new RegExp(this.$base.url,"g"),"");
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yunshugenzong/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yunshugenzongCrossAddOrUpdateFlag = false;
											this.parent.search();
			...
			yunshutupianUploadChange(fileUrls) {
				this.ruleForm.yunshutupian = fileUrls;
			},
		}
	};
</script>
