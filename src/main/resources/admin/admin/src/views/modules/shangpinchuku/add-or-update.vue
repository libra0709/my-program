	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info' && !ro.chukutupian" label="出库图片" prop="chukutupian" >
					<file-upload
						tip="点击上传出库图片"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.chukutupian?ruleForm.chukutupian:''"
						@change="chukutupianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item class="upload" v-else-if="ruleForm.chukutupian" label="出库图片" prop="chukutupian" >
					<img v-if="ruleForm.chukutupian.substring(0,4)=='http'&&ruleForm.chukutupian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.chukutupian" width="100" height="100">
					<img v-else-if="ruleForm.chukutupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.chukutupian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.chukutupian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="出库数量" prop="kucunshuliang" >
					<el-input v-model.number="ruleForm.kucunshuliang" placeholder="出库数量" clearable  :readonly="ro.kucunshuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="出库数量" prop="kucunshuliang" >
					<el-input v-model="ruleForm.kucunshuliang" placeholder="出库数量" readonly></el-input>
				</el-form-item>
			...
			<el-form-item v-else-if="ruleForm.chukubeizhu" label="出库备注" prop="chukubeizhu" >
				<span class="text">{{ruleForm.chukubeizhu}}</span>
			</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='chukutupian'){
							this.ruleForm.chukutupian = obj[o];
							this.ro.chukutupian = true;
							continue;
						}
						if(o=='kucunshuliang'){
			...
			// 提交
			async onSubmit() {
					if(this.ruleForm.chukutupian!=null) {
						this.ruleForm.chukutupian = this.ruleForm.chukutupian.replace(new RegExp(this.$base.url,"g"),"");
					}
			...
			chukutupianUploadChange(fileUrls) {
				this.ruleForm.chukutupian = fileUrls;
			},
		}
	};
</script>
