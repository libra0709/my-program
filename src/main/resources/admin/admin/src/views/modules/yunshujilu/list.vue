<template>
	<div class="main-content" :style='{"padding":"20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					...
					<div :style='{"alignItems":"center","margin":"0 5px 12px 0","display":"flex"}' class="select" label="运输状态" prop="yunshuzhuangtai">
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#000","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">运输状态</label>
					...
					<div :style='{"alignItems":"center","margin":"0 5px 12px 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#000","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">起点</label>
						<el-input v-model="searchForm.qidian" placeholder="起点" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 5px 12px 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#000","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">终点</label>
						<el-input v-model="searchForm.zhongdian" placeholder="终点" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 5px 12px 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#000","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">司机姓名</label>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yunshujilu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('yunshujilu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yunshujilu','配送率占比')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						配送率占比
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column  :resizable='true' prop="yunshutupian" width="200" label="运输图片">
						<template slot-scope="scope">
							<div v-if="scope.row.yunshutupian">
								<img v-if="scope.row.yunshutupian.substring(0,4)=='http'&&scope.row.yunshutupian.split(',w').length>1" :src="scope.row.yunshutupian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.yunshutupian)">
								<img v-else-if="scope.row.yunshutupian.substring(0,4)=='http'" :src="scope.row.yunshutupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.yunshutupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.yunshutupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.yunshutupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true' prop="qidian" label="起点">
						<template slot-scope="scope">
							{{ scope.row.qidian?scope.row.qidian.split(';')[2]:'' }}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="zhongdian" label="终点">
						<template slot-scope="scope">
							{{ scope.row.zhongdian?scope.row.zhongdian.split(';')[2]:'' }}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="dizhi"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yunshujilu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('yunshujilu','监控')" @click="yunshugenzongCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								监控
							</el-button>
							<el-button class="btn8" v-if="isAuth('yunshujilu','异常')" @click="yichangchuliCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								异常
							</el-button>
							<el-button class="edit" v-if=" isAuth('yunshujilu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="btn8" type="success" @click="showClick(scope.row)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								地图
							</el-button>
							<el-button class="del" v-if="isAuth('yunshujilu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<yunshugenzong-cross-add-or-update v-if="yunshugenzongCrossAddOrUpdateFlag" :parent="this" ref="yunshugenzongCrossaddOrUpdate"></yunshugenzong-cross-add-or-update>
		<yichangchuli-cross-add-or-update v-if="yichangchuliCrossAddOrUpdateFlag" :parent="this" ref="yichangchuliCrossaddOrUpdate"></yichangchuli-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="yunshuzhuangtaiChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import yunshugenzongCrossAddOrUpdate from "../yunshugenzong/add-or-update";
	import yichangchuliCrossAddOrUpdate from "../yichangchuli/add-or-update";
	...
	export default {
		data() {
			return {
				...
				yunshugenzongCrossAddOrUpdateFlag: false,
				yichangchuliCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			yunshugenzongCrossAddOrUpdate,
			yichangchuliCrossAddOrUpdate,
			tMap,
		},
		methods: {
			mapClose(){
			...
			showClick(row){
				...
				let waypoints = []
				if(row.qidian&&row.qidian.split(';').length>1) {
					longitude1 = row.qidian.split(';')[0]
					latitude1 = row.qidian.split(';')[1]
				}
				if(row.zhongdian&&row.zhongdian.split(';').length>1) {
					longitude2 = row.zhongdian.split(';')[0]
					latitude2 = row.zhongdian.split(';')[1]
				}
			...
			yunshugenzongCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yunshugenzongCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yunshujilu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yunshugenzongCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yunshugenzongCrossaddOrUpdate.init(row.id,type);
				});
			},
			yichangchuliCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yichangchuliCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yunshujilu');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yichangchuliCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yichangchuliCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var yunshuzhuangtaiChart1 = echarts.init(document.getElementById("yunshuzhuangtaiChart1"),'macarons');
					this.$http({
						url: "yunshujilu/group/yunshuzhuangtai",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].yunshuzhuangtai);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].yunshuzhuangtai
								})
							}
							var option = {};
							...
							// 使用刚指定的配置项和数据显示图表。
							yunshuzhuangtaiChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								yunshuzhuangtaiChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			init () {
				this.yunshuzhuangtaiOptions = "已完成,配送中".split(',')
			},
			search() {
				this.pageIndex = 1;
				this.getDataList();
