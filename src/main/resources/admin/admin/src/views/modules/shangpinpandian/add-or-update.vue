	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info' && !ro.pandiantupian" label="盘点图片" prop="pandiantupian" >
					<file-upload
						tip="点击上传盘点图片"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.pandiantupian?ruleForm.pandiantupian:''"
						@change="pandiantupianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item class="upload" v-else-if="ruleForm.pandiantupian" label="盘点图片" prop="pandiantupian" >
					<img v-if="ruleForm.pandiantupian.substring(0,4)=='http'&&ruleForm.pandiantupian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.pandiantupian" width="100" height="100">
					<img v-else-if="ruleForm.pandiantupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.pandiantupian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.pandiantupian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				...
				<el-form-item class="input" v-if="type!='info'"  label="盘点数量" prop="pandianshuliang" >
					<el-input v-model.number="ruleForm.pandianshuliang" placeholder="盘点数量" clearable  :readonly="ro.pandianshuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="盘点数量" prop="pandianshuliang" >
			...
			<el-form-item class="textarea" v-if="type!='info'" label="盘点备注" prop="pandianbeizhu" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.pandianbeizhu" label="盘点备注" prop="pandianbeizhu" >
				<span class="text">{{ruleForm.pandianbeizhu}}</span>
			</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='pandiantupian'){
							this.ruleForm.pandiantupian = obj[o];
							this.ro.pandiantupian = true;
							continue;
						}
						if(o=='kucunshuliang'){
			...
			// 提交
			async onSubmit() {
					if(this.ruleForm.pandiantupian!=null) {
						this.ruleForm.pandiantupian = this.ruleForm.pandiantupian.replace(new RegExp(this.$base.url,"g"),"");
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shangpinpandian/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shangpinpandianCrossAddOrUpdateFlag = false;
											this.parent.search();
			...
			pandiantupianUploadChange(fileUrls) {
				this.ruleForm.pandiantupian = fileUrls;
			},
		}
	};
</script>
