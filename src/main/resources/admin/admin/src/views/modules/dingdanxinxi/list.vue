<template>
	<div class="main-content" :style='{"padding":"20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					...
					<div :style='{"alignItems":"center","margin":"0 5px 12px 0","display":"flex"}' class="select" label="订单状态" prop="dingdanzhuangtai">
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#000","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">订单状态</label>
						<el-select clearable v-model="searchForm.dingdanzhuangtai" placeholder="请选择订单状态" >
							<el-option v-for="(item,index) in dingdanzhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('dingdanxinxi','销售金额')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						销售金额
					</el-button>
				</el-row>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('dingdanxinxi','分配')" @click="yunshurenwuCrossAddOrUpdateHandler(scope.row,'cross','','','dingdanzhuangtai','已接单','已接单,待接单'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								分配
							</el-button>
		...
		<yunshurenwu-cross-add-or-update v-if="yunshurenwuCrossAddOrUpdateFlag" :parent="this" ref="yunshurenwuCrossaddOrUpdate"></yunshurenwu-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="goumaifeiyongChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import yunshurenwuCrossAddOrUpdate from "../yunshurenwu/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				yunshurenwuCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			yunshurenwuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yunshurenwuCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yunshurenwuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yunshurenwuCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var goumaifeiyongChart1 = echarts.init(document.getElementById("goumaifeiyongChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							goumaifeiyongChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								goumaifeiyongChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
