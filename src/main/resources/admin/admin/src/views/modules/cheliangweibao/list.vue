		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					...
					<div :style='{"alignItems":"center","margin":"0 5px 12px 0","display":"flex"}'>
						...
						<el-input v-model="searchForm.zaizhongleixing" placeholder="载重类型" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 5px 12px 0","display":"flex"}' class="select" label="维保进度" prop="weibaojindu">
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#000","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">维保进度</label>
						<el-select clearable v-model="searchForm.weibaojindu" placeholder="请选择维保进度" >
							<el-option v-for="(item,index) in weibaojinduOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('cheliangweibao','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('cheliangweibao','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						删除
					</el-button>
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column  :resizable='true' prop="weibaotupian" width="200" label="维保图片">
						<template slot-scope="scope">
							<div v-if="scope.row.weibaotupian">
								<img v-if="scope.row.weibaotupian.substring(0,4)=='http'&&scope.row.weibaotupian.split(',w').length>1" :src="scope.row.weibaotupian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.weibaotupian)">
								<img v-else-if="scope.row.weibaotupian.substring(0,4)=='http'" :src="scope.row.weibaotupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.weibaotupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.weibaotupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.weibaotupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="维保时间">
						<template slot-scope="scope">
							{{scope.row.weibaoshijian}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('cheliangweibao','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('cheliangweibao','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('cheliangweibao','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				if(this.searchForm.weibaojindu!='' && this.searchForm.weibaojindu!=undefined){
					params['weibaojindu'] = this.searchForm.weibaojindu
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "cheliangweibao/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
