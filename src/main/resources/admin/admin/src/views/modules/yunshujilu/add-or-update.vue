	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="运输状态" prop="yunshuzhuangtai" >
				...
				<el-form-item v-else class="input" label="运输状态" prop="yunshuzhuangtai" >
				...
				<el-form-item class="select" v-if="type!='info'" label="车辆名称" prop="cheliangmingcheng" >
				...
				<el-form-item class="input" v-else-if="ruleForm.cheliangmingcheng" label="车辆名称" prop="cheliangmingcheng" >
			...
			<el-form-item class="textarea" v-if="type!='info'" label="运输详情" prop="yunshuxiangqing" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="运输详情"
					v-model="ruleForm.yunshuxiangqing" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.yunshuxiangqing" label="运输详情" prop="yunshuxiangqing" >
				<span class="text">{{ruleForm.yunshuxiangqing}}</span>
			</el-form-item>
	...
	export default {
		...
		mounted() {
			...
			VueAMap.initAMapApiLoader({
				key: 'f04ebb18061e825c758c2517b235954f',
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='yunshuxiangqing'){
							this.ruleForm.yunshuxiangqing = obj[o];
							this.ro.yunshuxiangqing = true;
							continue;
						}
						if(o=='sijizhanghao'){
				...
				this.$http({
					url: `option/cheliangdiaodu/cheliangmingcheng`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			cheliangmingchengChange () {
				this.$http({
					url: `follow/cheliangdiaodu/cheliangmingcheng?columnValue=`+ this.ruleForm.cheliangmingcheng,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						...
						if(data.data.chepaihaoma){
							this.ruleForm.chepaihaoma = data.data.chepaihaoma
						}
					} else {
						this.$message.error(data.msg);
					}
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yunshujilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yunshujiluCrossAddOrUpdateFlag = false;
											this.parent.search();
