<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item v-else-if="ruleForm.fenpeixiangqing" label="分配详情" prop="fenpeixiangqing" >
				<span class="text">{{ruleForm.fenpeixiangqing}}</span>
			</el-form-item>
			<el-form-item class="textarea" v-if="type!='info'" label="规划路线" prop="guihualuxian" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="规划路线"
					v-model="ruleForm.guihualuxian" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.guihualuxian" label="规划路线" prop="guihualuxian" >
				<span class="text">{{ruleForm.guihualuxian}}</span>
			</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='guihualuxian'){
							this.ruleForm.guihualuxian = obj[o];
							this.ro.guihualuxian = true;
							continue;
						}
					}
				}
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yunshurenwu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yunshurenwuCrossAddOrUpdateFlag = false;
											this.parent.search();
